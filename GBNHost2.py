...
class GBNHost():
    ...
    def make_pkt(self, ACK_flag, seq_num, ack_num, payload = None, checksum=None):    
        if payload is not None and checksum is None:
            packet = pack("!ii?i%is" % len(payload), seq_num, ack_num, ACK_flag, len(payload), payload.encode())
            checksum = self.checksum(packet)
            packet = pack("!iiH?i%is" % len(payload), seq_num, ack_num, checksum, ACK_flag, len(payload), payload.encode())
            return packet
        else:
            packet = pack('!ii?i', 0, ack_num, ACK_flag, 0)
            checksum = self.checksum(packet)
            packet = pack("!iiH?i", 0, ack_num, checksum, ACK_flag, 0)
            return packet
    ...
    def receive_from_application_layer(self, payload):
        if self.next_seq_num < self.window_base + self.window_size:
            packet = self.make_pkt(False, self.next_seq_num, 0, payload, None)
            header = unpack("!iiH?i", packet[:15])  
            payload = unpack("!%is"%header[4], packet[15:])[0].decode() 
            self.unACKed_buffer[self.next_seq_num] =  packet
            self.simulator.pass_to_network_layer(self.entity, packet, False)
    ...
    def receive_from_network_layer(self, byte_data):
        try:
            ...
            if self.corrupted(byte_data) == True:
